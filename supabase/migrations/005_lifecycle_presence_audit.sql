...
begin
  ...
  if tg_op = 'DELETE' then
    -- Workspace deletion cascades through memberships. The parent workspace and
    -- its audit history are being removed, so do not insert a new child row.
...
begin
  ...
  if tg_op = 'DELETE' then
    -- Skip board events generated by a cascading workspace deletion.
    if pg_trigger_depth() > 1 then return old; end if;
    perform public.append_workspace_audit_event(
      old.workspace_id,
      'board_deleted',
