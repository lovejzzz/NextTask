...
- ~~Wire Gemma 4's function-calling through the action gates on the in-browser path~~ ✅ **the live in-browser agent**: chat → tool call → gate → Accept/Dismiss consent card → reversible execute (single action + multi-step plan w/ rollback), few-shot prompt, prompt-injection test, glass-box trail, hybrid policy, agent self-test. Built per `docs/design/agent-rung-plan.md`; live Gemma quality still pending a WebGPU run (`RUBRIC.md`).
- ~~The self-improvement loop: plan → execute → review → plan next~~ ✅ closed the missing **review** keystone — `reviewOutcome.ts` scores whether an executed plan/action actually did what it said (pure, deterministic, per-`ActionKind` rubric), `loop.ts` decides honestly whether to offer another round (goal-met / loop-until-dry / a hard round cap), narrated in chat. Three independent designs were adversarially verified first; all three tried some form of letting a later round skip the consent click, which was rejected outright — every round still needs its own Accept. See `docs/design/self-improvement-loop-plan.md`.
